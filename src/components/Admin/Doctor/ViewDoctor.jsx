...
const ViewDoctor = (props) => {
    ...
    const sortedSchedules = dataDetailDoctor?.thoiGianKham.sort(
    ...
    const scheduleItems1 = sortedSchedules.map((schedule) => (
        <div key={schedule._id}>
            <span
                style={{ fontWeight: "bold", fontSize: "18px", color: "navy" }}
            >
            ...
            <Row gutter={[10, 10]}>
                {schedule.thoiGianId
                    .sort((a, b) => {
                        ...
                        return moment(startA, "HH:mm").diff(
                            moment(startB, "HH:mm")
                        );
                    })
                    .map((timeSlot) => (
    ...
    const scheduleItems = dataDetailDoctor?.thoiGianKham
        ...
        .slice(
            0,
            isExpanded ? dataDetailDoctor.thoiGianKham.length : visibleCount
        ) // Lấy số lượng lịch theo trạng thái hiển thị
        .map((value) => (
            <div key={value._id}>
                <span
                    style={{
    ...
    const scheduleItems3 =
        dataDetailDoctor.thoiGianKham.length > 0 ? (
    ...
    const handleDeleteDateCu = async () => {
        try {
            ...
            setDataDetailDoctor((prevData) => ({
                ...prevData,
                thoiGianKham: prevData.thoiGianKham.filter((slot) =>
    ...
    const items = [
        ...
        },
        {
            ...
            children: (
                <Badge
                    status="processing"
                    text={`${dataDetailDoctor?.roleId.key}`}
                />
            ),
            span: 2,
        },
            ...
            children: (
                <>
                    {dataDetailDoctor?.chucVuId.map((value, index) => (
                        <div key={value._id}>
                            {value.name}
                            {index < dataDetailDoctor.chucVuId.length - 1 && (
                                <br />
                            )}{" "}
                            {/* Thêm <br /> nếu không phải là phần tử cuối */}
                        </div>
                    ))}
                </>
            ),
                ...
                <>
                    {dataDetailDoctor?.chuyenKhoaId.map((value, index) => (
                        <div key={value._id}>
                            {value.name}
                            {index <
                                dataDetailDoctor.chuyenKhoaId.length - 1 && (
                                <br />
                            )}{" "}
                            {/* Thêm <br /> nếu không phải là phần tử cuối */}
                        </div>
        ...
        {
            key: "giaKhamNuocNgoai",
            span: 2,
            label: "Giá khám Người Nước Ngoài",
            children: (
                <div style={{ color: "red" }}>
                    {formatCurrencyEn(dataDetailDoctor?.giaKhamNuocNgoai)}
                </div>
            ),
        },

        {
            ...
            children: (
                <>
                    <div>
                        {scheduleItems}
                        {dataDetailDoctor?.thoiGianKham.length > visibleCount &&
                            !isExpanded && (
                        ...
                        {!isExpanded &&
                            dataDetailDoctor?.thoiGianKham.length > 3 && (
                                <Button
                                    type="link"
    ...
    return (
        <Drawer
            title={
                dataDetailDoctor
                    ? `Thông tin chi tiết của Bác sĩ ${dataDetailDoctor.lastName} ${dataDetailDoctor.firstName}`
                    : "Thông tin bác sĩ"
            }
            placement={placement}
            width={900}
